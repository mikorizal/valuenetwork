...
class NewGardenForm(forms.Form):
    garden_name = forms.CharField(widget=forms.TextInput(attrs={'class': 'input-xxlarge',}))
    address = forms.CharField(required=True, widget=forms.TextInput(attrs={'class': 'input-xxlarge',}))
    choose_known_provider = AgentModelChoiceField(
        queryset=EconomicAgent.objects.all(),
        label="Choose a known provider",
        required=False,
        widget=forms.Select(attrs={'class': 'known-provider provider-group chzn-select'}))
    or_add_new_provider  = forms.CharField(widget=forms.TextInput(attrs={'class': 'new-provider provider-group input-xxlarge',}),
        help_text=_("If known provider chosen, new provider will be ignored."),
        required=False)
    new_provider_type = forms.ModelChoiceField(
        queryset=AgentType.objects.all(),
        empty_label=None,
        widget=forms.Select(
            attrs={'class': 'chzn-select'}))
    size_in_sq_ft = forms.DecimalField(widget=forms.TextInput(attrs={'class': 'quantity input-small',}))
    show_on_public_map = forms.BooleanField(
        required=False,
...
class CreateEconomicResourceForm(forms.ModelForm):
    ...
    identifier = forms.CharField(
        ...
        #help_text="For example, lot number or serial number.",
        widget=forms.TextInput(attrs={'class': 'item-name',}))
    #todo: this will need some changes when the gardens gang
    # wants to create resouurces of other types...
